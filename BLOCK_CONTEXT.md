...
| `risk_gate` | `rtl/risk_gate.sv` | Implemented with constant stand-in limits because risk tables and global kill config input are absent from the frozen interface. Uses OR-masked kill-reason generation for parallel checks. | Verilator lint-only passes. `tb/tb_risk_gate.sv` lint-only passes. `rtl/risk_gate_assertions.sv` lint-only passes with assertions enabled. Existing VCD: `tb/risk_gate_smoke.vcd`. |
...
| Assertion bind files | `rtl/*_assertions.sv` | Complete for existing RTL modules: `mac_shim`, `hdr_stripper`, `field_aligner`, `sym_id_mapper`, `risk_gate`, and `pkt_formatter`. | All assertion bind files lint-only pass standalone where applicable, with existing smoke tests where available, and through `hft_engine`. |
...
- `risk_gate`: simultaneous violation priority is unspecified.
